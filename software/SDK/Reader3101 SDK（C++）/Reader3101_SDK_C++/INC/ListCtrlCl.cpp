...
CListCtrlCl::~CListCtrlCl()
{
	...
	for (pos = m_ptrListCol.GetHeadPosition();pos!=NULL;)
	{
	...
	for (pos = m_ptrListItem.GetHeadPosition();pos!=NULL;)
	{
	...
	for (pos = m_colTextColor.GetHeadPosition();pos!=NULL;)
	{
	...
	for (pos = m_ItemTextColor.GetHeadPosition();pos!=NULL;)
	{
	...
	m_ptrListCol.RemoveAll();  //保存列颜色
	m_ptrListItem.RemoveAll(); //保存Item颜色表
	m_colTextColor.RemoveAll(); //保存列字体颜色
	m_ItemTextColor.RemoveAll(); //保存单元格字体颜色
}
...
void CListCtrlCl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if( (GetStyle() & LVS_TYPEMASK) == LVS_REPORT )
	{
		...
		/*** 加这个******************/
		CPen newPen, *oldPen;
		newPen.CreatePen(PS_SOLID, 1, RGB(0,0,255));//RGB(255,0,0));
