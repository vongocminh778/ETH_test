...
void CMyColorList::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	for(int i=0;i<m;i++)
	{
		...
		{
		case LVCFMT_LEFT:
			rcItem.left=rcItem.left+5;	//add by martrin 20130701
...
void CMyColorList::InsertItem(int nItem, LPCTSTR lpszItem)
{
	int m=GetItemCount();	
	...
	//col->colSelect=RGB(52,148,39);
	col->colBack=RGB(0xC6,0xD5,0xFD);//RGB(12,255,23);
	col->colText=RGB(187,0,222);
	col->colSelect=RGB(52,148,39);
	m_ArrayCol.InsertAt(m,(CObject *)col);
